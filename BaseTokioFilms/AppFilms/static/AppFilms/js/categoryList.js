document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(`${urls.updateCategoryName}${categoryId}/`, {
      ...
      .then((data) => {
        if (data.status === "success") {
          ...
          updateCategorySelect(categoryId, newName); // Actualizar el select
      ...
      fetch(`${urls.deleteCategory}${categoryIdToDelete}/`, {
        ...
        .then((data) => {
          if (data.status === "success") {
            ...
            document.querySelector("#modal2 input").value = ""; // Limpiar el input al cerrar el modal
            modal2.classList.add("hidden");
            removeCategoryFromSelect(categoryIdToDelete); // Eliminar del select
            actualizarLista(); // Actualizar la lista
          } else {
  ...
  // Función para añadir una categoría al select
  ...
  // Función para actualizar una categoría en el select
  function updateCategorySelect(id, newName) {
    const option = document.getElementById("category").querySelector(`option[value="${id}"]`);
    if (option) {
      option.textContent = newName;
    }
  }

  // Función para eliminar una categoría del select
  function removeCategoryFromSelect(id) {
    const option = document.getElementById("category").querySelector(`option[value="${id}"]`);
    if (option) {
      option.remove();
    }
  ...
  // Función para actualizar la lista de categorías en el select
  function actualizarLista() {
    fetch("/lista_items") // Llama a la vista Django
